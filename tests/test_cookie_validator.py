...
try:
    from fastapi_assets.request_validators.cookie_validator import CookieValidator as CookieAssert
...
validate_length_5 = CookieAssert(
    alias="code", min_length=5, max_length=5, on_length_error_detail="Code must be 5 chars."
)
...
validate_custom_role = CookieAssert(
    alias="role", validators=[_custom_check], on_validator_error_detail="Invalid role."
)
...
def _async_validator(val: str):
    ...
    if not val.startswith("valid_"):
        raise ValidationError("Token must start with 'valid_'.")
...
validate_email_format = CookieAssert(
    alias="user-email", format="email", on_pattern_error_detail="Invalid email format."
)
...
async def test_custom_validator_fail(client: AsyncClient):
    ...
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert "role is invalid" in response.json()["detail"].lower()
